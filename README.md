...
Во время записи каждые 60 секунд в лог выводится статус: количество собранных сообщений, время старта, текущее время и прошедшее время.
...
Все временные метки в интерактивном режиме отображаются как абсолютные UTC-даты (например, `2026-03-30 15:45:32.123 UTC`).
...
- `t` — ручной ввод времени UTC в формате `YYYY-MM-DD HH:MM:SS.mmm` (полный) или `HH:MM:SS.mmm` / `MM:SS` / `SS` (дата определяется автоматически)
...
При наличии SSH-туннеля он автоматически устанавливается через `ssh -L` перед подключением к Redis и закрывается по завершении записи. Команда SSH логируется, что позволяет запустить её вручную для отладки в случае проблем с подключением.
